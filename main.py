...
def train():
    ...
    try:
        resume_episode, completions, base_name = mc_agent.load_checkpoint(
            checkpoint_path, memories_path
        )
    ...
    for episode in tqdm(range(resume_episode, episodes), desc="Episodes", position=0):
        ...
        with tqdm(
            total=episodemaxsteps if episodemaxsteps > 0 else None,
            desc="Episode steps",
            position=1,
            leave=False,
        ) as step_bar:
            while not done and (episodemaxsteps <= 0 or steps < episodemaxsteps):
                ...
                mc_agent.replay_buffer.add_memory(
                    (
                        state,
                        action,
                        reward,
                        experience_buffer.get_stacked_frames(),
                        done,
                    )
                )
        ...
        if episode % checkpoint_interval == 0:
            mc_agent.create_checkpoint(
                checkpoint_path, memories_path, episode, completions, base_name
            )
...
def evaluate():
    ...
    number_of_agents = len(
        mission.findall("{http://ProjectMalmo.microsoft.com}AgentSection")
    )
    ...
    threads = [
        Thread(
            target=run_evaluate,
            args=(i, global_stop_event, agent_done_event, xml, mc_agent),
        )
...
def run_evaluate(role, global_stop_event, agent_done_event, xml, mc_agent):
    ...
    while not global_stop_event.is_set():
        ...
        while (
            ...
            and (episodemaxsteps <= 0 or steps < episodemaxsteps)
        ):
            steps += 1
            ...
            if role == 0:  # Agent
                ...
                if next_obs is None or next_obs.size == 0:
                    print(
                        f"Warning: Encountered empty observation at step {steps}. Ending episode early."
                    )
                    done = True
                ...
                total_reward += reward
                log(
                    f"Step {steps}: Action={action}, Reward={reward}, Total reward={total_reward}, Done={done}"
                )
...
def start_tensorboard(log_dir=None, download=False):
    ...
    if download:
        ...
        try:
            ...
            gdown.download(
                log_url, output_path, fuzzy=True, use_cookies=False, quiet=False
            )
    ...
    try:
        ...
        print(
            f"TensorBoard is running on http://localhost:6006 with logs from {log_dir}"
        )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Number of memories (steps, each: current 4 frames & latest 3 + new frame) for replay buffer (needs to be adjusted to fit RAM-size)
    replay_size = 150000
    ...
    parser = argparse.ArgumentParser(
        description="Train or evaluate the Minecraft agent and visualize the training progress with TensorBoard."
    )

    parser.add_argument(
        "--train", action="store_true", help="Train the Minecraft agent"
    )

    parser.add_argument(
        "--eval", action="store_true", help="Evaluate the Minecraft agent"
    )
    ...
    parser.add_argument(
        "--download",
        action="store_true",
        help="Download TensorBoard logs from Google Drive",
    )

    parser.add_argument(
        "--logdir", type=str, default=None, help="Directory for TensorBoard logs"
    )
